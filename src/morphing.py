...
def elasticity(mesh2D, material2D, boundarys, boundary_scale = 1.0):
    ...
    p, t, adj = mesh2D.nodes, mesh2D.elements, mesh2D.adjoint
    ...
    constructive = material2D.constructive
    ...
    # just cache for test
    ks0 = stiffness.generate_stiffness_matrix_k0(p, t, constructive, basis, jacobis)
    pickle.dump(ks0, open(f"stiff-elasticity-k0s-{n_elements}-elements.bin", "wb"))
    as0 = pickle.load(open(f'stiff-elasticity-k0s-{n_elements}-elements.bin', "rb"))
    # if not os.path.exists(f"stiff-elasticity-k0-{n_elements}-elements.bin"):
    #     k0 = stiffness.mapping_element_stiffness_matrix(p, t, basis, as0)
    #     pickle.dump(k0, open(f"stiff-elasticity-k0-{n_elements}-elements.bin", "wb"))
    # a0 = pickle.load(open(f"stiff-elasticity-k0-{n_elements}-elements.bin", "rb"))
    
    # or don't cache stiffness matrix
    # as0 = stiffness.generate_stiffness_matrix_k0(p, t, constructive, basis, jacobis)
...
def morphing_phase(phase_id: int, runtime_id: int, mesh2D, material2D, boundarys, boundary_scale = 1.0, s_crit = 1.1, l_inst = 0.015):
    n_nodes, n_elements = mesh2D.n_nodes, mesh2D.n_elements
    p, t, adj = mesh2D.nodes, mesh2D.elements, mesh2D.adjoint
    related = mesh2D.bonds
    centers = mesh2D.centers
    ...
    lame_lambda = material2D.lame_lambda
    constructive = material2D.constructive
    c0 = material2D.coefficients[0]
    coef_c0 = np.vectorize(lambda xi2: c0 * np.exp(-xi2**0.5 / l_inst))
    ...
    # just cache for test
    if not os.path.exists(f"stiff-elasticity-k0s-{n_elements}-elements.bin"):
        ks0 = stiffness.generate_stiffness_matrix_k0(p, t, constructive, basis, jacobis)
        pickle.dump(ks0, open(f"stiff-elasticity-k0s-{n_elements}-elements.bin", "wb"))
    as0 = pickle.load(open(f'stiff-elasticity-k0s-{n_elements}-elements.bin', "rb"))
    ...
    is_first = True
    connection = np.ones(shape=(n_elements, n_elements, n_gauss, n_gauss), dtype=np.bool)
    broken_endpoint = []
    all_broken_endpoint = []
    for ii in range(MAX_ITER):
        ...
        b = assemble_load_vector(n_nodes)
        compiled_boundary = treat_boundary.complie_boundary(p, boundarys)
        treat_boundary.apply_boundary(a, b, p, compiled_boundary, boundary_scale)
        # print(a[0:5, 0:5])
        u = solve_linear_system(a, b)
        broken_endpoint, cnt = pd_stiffness.deal_bond_stretch(p, t, related, weight_function_handle, connection, u, apd, coef_c0, basis, s_crit)
        all_broken_endpoint.extend(broken_endpoint)
        is_first = False
        tot = time.time() - t0
        print(f"    Phase {phase_id:2d}, Runtime {runtime_id:2d}, Iteration {ii:4d}: ", end="")
        print(f"total broken bonds {cnt:8d}, total time cost {utils.formatting_time(tot)}")
        # input()
...
def morphing_setup(mesh2D, material2D, boundarys):
    ...
    w_element = postprocessing.convert_distortion_energy_for_element(w_distortion, t)
    critical_indices = postprocessing.maximum_distortion_energy_criterion(w_element, MAX_DISTORTION_ENERGY)
...
def morphing(mesh2D, material2D, boundarys):
    ...
    for phase in range(TOTAL_PHASES):
        ...
        while flag:
            flag = False
            u, all_broken_endpoint = morphing_phase(phase, runtime_id, mesh2D, material2D, boundarys, boundary_scale)
            ...
            print(f"    update mesh, with DGFE/tot=({n_dg_elements}/{n_elements})")
            runtime_id += 1
        print(f"Phase {phase:4d} end, total time cost {utils.formatting_time(time.time() - t0)}")
        ...
        postprocessing.export_tecplot_data(f"{example_name}-morphing-phase-{phase:02d}-{n_elements}-elements.dat", tecplot_config, p, t, u, u_abs, epsilon, sigma, w_distortion)
        ...
        postprocessing.export_tecplot_data(f"{example_name}-morphing-phase-{phase:02d}-{n_elements}-elements-deform.dat", deform_config, p_deform, t, u, u_abs)
        ...
        postprocessing.export_tecplot_data(f"{example_name}-morphing-phase-{phase:02d}-{n_elements}-elements-alpha-roughly.dat", alpha_config, p, t, w_rough)

        w_exact = meshdata.get_weight_function_value_exactly((w_, x_gauss, y_gauss), basis)
        alpha_config = postprocessing.generate_tecplot_config(n_nodes, n_elements, basis.length)
        alpha_config["variables"] = "X, Y, alpha"
        postprocessing.export_tecplot_data(f"{example_name}-morphing-phase-{phase:02d}-{n_elements}-elements-alpha-exactly.dat", alpha_config, p, t, w_exact)

    print(f"total phase {TOTAL_PHASES}, total time cost {utils.formatting_time(time.time() - start_timestamp)}")