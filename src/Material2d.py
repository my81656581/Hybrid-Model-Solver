...
class Material2d(object):
    def __init__(self,
        ...
        self.__lame_mu_ = youngs_modulus / (1 + poissons_ratio) / 2.0
        self.__lame_lambda_ = c33 * 2.0 * poissons_ratio / (1 - 2 * poissons_ratio)
        self.__constructive_ = np.array([[c11, c12, 0], [c12, c11, 0], [0, 0, c33]])
...
class PdMaterial2d(Material2d):
    def __init__(self, continuum: Material2d=Material2d(), coefficients=np.array([1.0, 0.0, 0.0])):
    ...
    def __init_std_meshgrid(self, horizon_radius, grid_size):
        ...
        n_nodes, n_element = len(nodes), len(elements)
        self.__mesh_ = PrototypePdMesh2d(n_nodes, n_element)
    ...
    def syncIsotropic(self, horizon_radius, grid_size, inst_len):
        ...
        p, e = self.__mesh_.nodes, self.__mesh_.elements
        related = self.__mesh_.bonds
        c0 = self.coefficients[0]
        coef_fun = np.vectorize(lambda xi2: c0 * np.exp(-xi2**0.5 / inst_len))
        pd_constructive = pd_stiffness.estimate_stiffness_matrix_isotropic(self.__mesh_, coef_fun)
