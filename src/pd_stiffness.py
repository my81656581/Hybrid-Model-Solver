...
def estimate_stiffness_matrix_isotropic(mesh, coef_fun):
    ...
    xy_local = [basis.transform(x_, y_, nodes[elements[i, :], :], (0, 0)) for i in range(n_elements)]
    ...
    return pd_constructive_ij


def estimate_stiffness_matrix(nodes, elements, related, coef_fun, basis, jacobis, material):
    w_, x_, y_ = gaussint.gauss_point_quadrature_standard()
    n_elements, n_gauss = len(elements), len(w_)
    xy_local = [basis.transform(x_, y_, nodes[elements[i, :], :], (0, 0)) for i in range(n_elements)]
    det_jacobi = stiffness.preprocessing_all_jacobi_det(n_elements, n_gauss, jacobis)
    ...
    for j in related[i]:
        ...
        for k in range(n_gauss):
            for l in range(n_gauss):
                ...
                scale = det_jacobi[i][k] * det_jacobi[j][l]
                pd_constructive_ij += scale * pd_constructive_core(xi_local[k], yi_local[k], xj_local[l], yj_local[l], coef_fun)
    pd_constructive_ij /= material.grid_vol
    print("i=", i)
    print("vertex_i=", nodes[elements[i, :], :])
    print("center_i=", np.mean(nodes[elements[i, :], :], 0))
    print("constructive=", (pd_constructive_ij[0], pd_constructive_ij[1], pd_constructive_ij[2], pd_constructive_ij[2]))
    material.coefficients[0] = material.constructive[0, 0] / pd_constructive_ij[0]
...
def generate_stiffness_matrix_k1(nodes, elements, related, weight_handle, coef_fun, basis, jacobis):
    w_, x_, y_ = gaussint.gauss_point_quadrature_standard()
...
def assemble_stiffness_matrix(nodes, elements, related, coef_fun, basis):
    ...
    xy_local = [basis.transform(x_gauss, y_gauss, nodes[elements[i, :], :], (0, 0)) for i in range(n_elements)]
    mapping = [np.reshape(np.hstack((elements[i, :], elements[i, :] + n_nodes)), (-1)) for i in range(n_elements)]
    jacobis = stiffness.preprocessing_all_jacobi(nodes, elements, basis)
    det_jacobi = stiffness.preprocessing_all_jacobi_det(n_elements, n_gauss, jacobis)
    shape0s = [np.reshape(basis.shape_vector(x_gauss[_], y_gauss[_]), (1, 4)) for _ in range(n_gauss)]
    shapes = [np.vstack((np.hstack((shape0s[_], zero1x4)), np.hstack((zero1x4, shape0s[_])))) for _ in range(n_gauss)]
    # time counter init begin
    flag, flag_0 = [0.17 * n_elements for _ in range(2)]
    t0 = time.time()
    # time counter init end
    for i in range(n_elements):
        # time counter runs begin
        if i > flag:
            flag = utils.display_progress(msg="        assembling stiffness martix pd",
                                          current=flag,
                                          display_sep=flag_0,
                                          current_id=i,
                                          total=n_elements,
        ...
        xi_local, yi_local = xy_local[i]
        mapping_i = mapping[i]
        ii_col, ii_row = [np.reshape(_, (-1)) for _ in np.meshgrid(mapping_i, mapping_i)]
        ...
        for j in related[i]:
            ...
            for k in range(n_gauss):
                ...
                for l in range(n_gauss):
                    ...
                    # scale = 1
                    # because of w_[_] == 1
                    core = scale * core_xi2(xi_local[k], yi_local[k], xj_local[l], yj_local[l], coef_fun)
...
def assemble_stiffness_matrix_with_weight(nodes, elements, related, weight_handle, coef_fun, basis):
    w_, x_gauss, y_gauss = gaussint.gauss_point_quadrature_standard()
    n_nodes, n_elements, n_gauss = len(nodes), len(elements), len(w_)
    ret = np.zeros(shape=(2 * n_nodes, 2 * n_nodes))
    zero1x4 = np.zeros(shape=(1, 4))
...
def deal_bond_stretch(nodes, elements, related, weight_handle, connection, displace_field, old_stiffness, coef_fun, basis, s_crit = 1.1):
    w_, x_gauss, y_gauss = gaussint.gauss_point_quadrature_standard()
    n_nodes, n_elements, n_gauss = len(nodes), len(elements), len(w_)
    zero1x4 = np.zeros(shape=(1, 4))
    endpoints, broken_bond_cnt = [], 0
    xy_local = [basis.transform(x_gauss, y_gauss, nodes[elements[i, :], :], (0, 0)) for i in range(n_elements)]
    uxuy_local = [basis.transform(x_gauss, y_gauss, displace_field[elements[i, :], :], (0, 0)) for i in range(n_elements)]
    mapping = [np.reshape(np.hstack((elements[i, :], elements[i, :] + n_nodes)), (-1)) for i in range(n_elements)]
    jacobis = stiffness.preprocessing_all_jacobi(nodes, elements, basis)
    det_jacobi = stiffness.preprocessing_all_jacobi_det(n_elements, n_gauss, jacobis)
    shape0s = [np.reshape(basis.shape_vector(x_gauss[_], y_gauss[_]), (1, 4)) for _ in range(n_gauss)]
    ...
    for i in range(n_elements):
        # time counter runs begin
        if i > flag:
            flag = utils.display_progress(msg="        dealing with bond stretch",
                                          current=flag,
                                          display_sep=flag_0,
                                          current_id=i,
                                          total=n_elements,
        ...
        for j in related[i]:
            ...
            mapping_j = mapping[j]
            jj_col, jj_row = [np.reshape(_, (-1)) for _ in np.meshgrid(mapping_j, mapping_j)]
            # jj_col, ii_row = [np.reshape(_, (-1)) for _ in np.meshgrid(mapping[j], mapping_i)]
            stiff_ii, stiff_ij, stiff_jj = [np.zeros(shape=(2 * n_gauss, 2 * n_gauss)) for _ in range(3)]
            # stiff_ii, stiff_ij = [np.zeros(shape=(2 * n_gauss, 2 * n_gauss)) for _ in range(2)]
            flag_j, weight_j = weight_handle(j)
            if flag_i == 0 and flag_j == 0: continue
            for k in range(n_gauss):
                ...
                for l in range(n_gauss):
                    ...
                    xi_old_x, xi_old_y = xi_local[k] - xj_local[l], yi_local[k] - yj_local[l]
                    xi_new_x, xi_new_y = uxi_local[k] - uxj_local[l] + xi_old_x, uyi_local[k] - uyj_local[l] + xi_old_y
                    xi_old_abs = np.sqrt(xi_old_x * xi_old_x + xi_old_y * xi_old_y)
                    xi_new_abs = np.sqrt(xi_new_x * xi_new_x + xi_new_y * xi_new_y)
                    ...
                    scale = (alpha_k + alpha_l) * 0.5 * det_jacobi[i][k] * det_jacobi[j][l] * 0.5
                    core = scale * core_xi2(xi_local[k], yi_local[k], xj_local[l], yj_local[l], coef_fun)
                    stiff_ii -= shape_k.T @ core @ shape_k
                    stiff_ij -= shape_k.T @ core @ shape_l
    ...
    # time counter summary begin
    tot = time.time() - t0
    print(f"        dealing with bond stretch completed. Total {utils.formatting_time(tot)}")
    # time counter summary end
